<template>
  <div>
    ...
    <button @click="refresh"
      ...
      <svg v-if="status === 'pending'" aria-hidden="true" role="status"
    ...
    <div class="grid md:grid-cols-3 sm:grid-cols-2 mt-8">
      <div v-for="cliente in clientes.clientes" :key="cliente.id"
        ...
        <div class="flex justify-center h-full max-h-40">
          <img :src="cliente.foto" :alt="cliente.nome" class="rounded-full w-40 object-cover border-8 border-teal-200">
        </div>
    ...
    <div class="flex justify-center items-center gap-5 mt-5 border-9">
      <button @click="fetchPreviousPage" :disabled="pagina === 1"
      ...
      <button @click="fetchNextPage" :disabled="pagina >= totalPages"
        class="bg-teal-500 text-white border-none py-2 px-3 rounded-md cursor-pointer transition ease-linear delay-75 hover:bg-teal-700 disabled:bg-teal-300 disabled:cursor-not-allowed">
        Próxima
      </button>
    </div>
